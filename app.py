...
# 2. Stylizacja CSS pasująca do usgvetscans.pl
st.markdown("""
    <style>
    .main-header {
...
# --- BANER GŁÓWNY W STYLU USG VET SCANS ---
st.markdown("""
    <div class="main-header">
        <h1>🩺 USG Vet Scans</h1>
        <p>Generator Opisów USG Weterynaryjnego</p>
    </div>
""", unsafe_allow_html=True)
...
if tryb == "🎙️ TRYB 1: Dyktowanie swobodne (Puste tło)":
    ...
    podyktowany_tekst = st.text_area(
        ...
        placeholder="np. Pęcherz moczowy miernie wypełniony ściana 2.5 mm, nerka lewa 4.5x2.8 cm, prawa 4.6x2.7 cm z przebudową pozapalną...",
        height=250
    )
...
# ==========================================
    ...
    st.markdown("---")
    st.subheader("📝 Odchylenia i Patologie (Szybkie Frazy)")
    st.caption("Kliknij przycisk, aby automatycznie wstawić najczęstszy opis patologii lub wpisz własny tekst.")

    # INICJALIZACJA SESSION_STATE DLA PÓL TEXT_AREA
    ...
    with c2:
        ...
        with col_t1:
            if st.button("➕ Zapalenie trzustki"):
                st.session_state['trzustka_pat'] = "lewy płat powiększony do 22 mm, obszar hipoechogenny 22x18.5 mm z miejscowym odczynem tłuszczowym"
        with col_t2:
    ...
    # --- GENEROWANIE TEKSTU KAPITULARNEGO ---
...
# --- WYŚWIETLANIE WYNIKU NA DOLE STRONY ---
st.subheader("📋 Wygenerowany Opis USG (Gotowy do skopiowania):")
st.text_area("Rozwiń lub kliknij w pole poniżej, naciśnij Ctrl+A, a następnie Ctrl+C:", value=final_report_text, height=350)
import streamlit as st
...
if tryb == "🎙️ TRYB 1: Dyktowanie swobodne (Puste tło)":
    ...
    podyktowany_tekst = st.text_area(
        "Podyktuj treść badania głosem:",
        placeholder="np. Pęcherz moczowy miernie wypełniony ściana 2.5 mm, nerka lewa 4.5x2.8 cm, prawa 4.6x2.7 cm z przebudową pozapalną, śledziona grubość 1.2 cm...",
        height=250
    )
...
# ==========================================
    ...
    st.caption("Wpisz wymiary narządów. Puste pola zostaną zastąpione wielokropkiem (...) wewnątrz szablonu normatywnego.")
    ...
    # Szablony narządowe z wielokropkami
    ...
    watroba_norma = f"Wątroba niepowiększona, miąższ gruboziarnisty, jednorodny, o prawidłowej echogeniczności, bez zmian ogniskowych, krawędzie narządu regularne. Naczynia wątrobowe nieposzerzone. Pęcherzyk żółciowy niepowiększony, ściana prawidłowej grubości, gr. ok. {val_pecherzyk} mm i echogeniczności, bez uchwytnych złogów w świetle. Drogi żółciowe nieposzerzone. Układ wrotny bez uchwytnych zmian w budowie."
    ...
    wezly_norma = "Węzły chłonne na terenie jamy brzusznej niepowiększone, bez uchwytnych zmian w budowie."
    plyn_norma = "Brak wolnego płynu w jamie brzusznej."
...
st.caption("Użyj ikony nakładających się kwadratów 📋 w prawym górnym rogu poniższego pola, aby natychmiast skopiować całą treść do schowka.")

# Okno z funkcją jednoklikowego kopiowania
